...
async function Blog({ searchParams }: props) {
  ...
  const blogOwners = await Promise.all(data.map((blog: any) => getUserById(blog.owner)));
        ...
        <div className={styles['content']}>
          <div className={styles['section1']}>
            ...
            {data?.slice(perPage * (page - 1), perPage * (page - 1) + perPage).map((item: any, index: any) => {
              ...
              return <BlogItem key={index} data={item} owner={blogOwners[index]} />;
            })}
            {/* Pagination bar */}
            <Pagiantion totalBlogs={data.length} page={page} perPage={perPage} />
          </div>
